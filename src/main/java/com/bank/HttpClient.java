...
import com.bank.business.validators.EmailValidator;
...
public class HttpClient {
    ...
    private static User performLogin(BankApiClient client, Scanner scanner) {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                var password = userNode.get("password").asText();
                ...
                var user = new User(username, got_email, password, id, isAdmin, true);
                var isAuthenticated = false;

                isAuthenticated = handleAuth(user, scanner);

                // In a real implementation, you would parse all user details from the response
                if (isAuthenticated) {
                    System.out.println("Login successful!");
                } else {
                    LOGGER.debug("Login failed with invalid password: {}",
                            Json.stringifyPretty(Json.toJson(response.body())));
    ...
    private static boolean handleAuth(User user, Scanner scanner) {
        var isAuthenticated = false;
        var authTries = 3;
        while (!isAuthenticated) {
            System.out.printf("Enter Password (%d tries left)\n", authTries);
            isAuthenticated = user.validatePassword(scanner.nextLine().trim());

            authTries--;
            if (authTries <= 0) {
                System.out.println("Login failed. because of invalid password");
                return false;
            }
        }
    ...
    private static User performRegistration(BankApiClient client, CliObjectMapper mapper) {
        ...
        try {
            ...
            if (response.statusCode() == 201) {
                ...
                var isAdmin = userNode.get("admin").asBoolean();
                var password = userNode.get("password").asText();
                LOGGER.debug("Registration Response Details: {}",
                        ResponseFormatter.formatJsonResponse(response.body()));

                var user = new User(username, email, password, id, isAdmin, true);
                client.setLoggedInUser(user);
                return user;
            } else {
