...
# Try to parse the token from the response
if echo "$LOGIN_RESPONSE" | grep -q '"token"'; then
    AUTH_TOKEN=$(echo "$LOGIN_RESPONSE" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
    echo "User login successful, token: $AUTH_TOKEN"
else
    echo "User login failed or token not found in response"
    kill $SERVER_PID 2>/dev/null
    exit 1
fi
...
ACCOUNT_RESPONSE=$(curl -s -w "%{http_code}" -X POST http://localhost:8080/accounts \
  ...
  -d '{"userId":1,"initialBalance":100.0}')
